        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-8">
          ...
          <div className="col-span-2 md:col-span-1 lg:col-span-2">
            ...
            <p className="text-[#71717a] text-sm leading-relaxed mb-6 max-w-xs">
              A plataforma de viagens mais moderna do Brasil. Descubra hotéis, destinos e pacotes inesquecíveis.
            </p>

            {/* Contact */}
            <div className="flex flex-col gap-2 mb-6">
              <a href="tel:+551140020022" className="flex items-center gap-2 text-[#71717a] hover:text-[#a78bfa] transition-colors text-sm">
                <Phone className="w-3.5 h-3.5" />
                0800 400 2002
              </a>
              <a href="mailto:suporte@hotelhub.com" className="flex items-center gap-2 text-[#71717a] hover:text-[#a78bfa] transition-colors text-sm">
          ...
          {Object.entries(LINKS).map(([title, links]) => (
            <div key={title}>
              <h4 className="text-white text-sm mb-4" style={{ fontWeight: 600 }}>{title}</h4>
      ...
      <div className="border-t border-[#1a1a1a]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-3">
          <p className="text-[#555] text-xs">
            © 2026 HotelHub. Todos os direitos reservados.
          </p>
          <div className="flex items-center gap-4">
            {["Visa", "Mastercard", "Pix", "Apple Pay"].map((method) => (
              <span key={method} className="text-[#555] text-xs bg-[#111111] border border-[#222222] px-2 py-1 rounded">
                {method}
              </span>
            ))}
          </div>
