...
import {
  ...
  Hotel, Plane, Package, Car, ChevronDown
} from "lucide-react";
...
const POPULAR_DESTINATIONS = [
  "Paris, França", "Bali, Indonésia", "Nova York, EUA", "Tóquio, Japão",
  "Maldivas", "Santorini, Grécia", "Roma, Itália", "Amsterdam, Holanda",
];
    ...
    return (
      <div className="flex items-center gap-2 bg-[#111111] border border-[#333333] rounded-2xl p-2">
        ...
        <div className="flex items-center gap-2 px-3">
          ...
          <input type="date" value={checkIn} onChange={e => setCheckIn(e.target.value)} className="bg-transparent text-[#a1a1aa] outline-none text-sm" />
        </div>
        <button
            ...
            {showDestSuggestions && filteredDests.length > 0 && (
              ...
              >
                {filteredDests.slice(0, 6).map((dest) => (
                  <button
                    key={dest}
                    onMouseDown={() => { setDestination(dest); setShowDestSuggestions(false); }}
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="text-left">
                ...
                <p className="text-sm text-white">{guests} {guests === 1 ? "hóspede" : "hóspedes"}</p>
              </div>
              <ChevronDown className={`w-4 h-4 text-[#71717a] ml-auto transition-transform ${showGuestPicker ? "rotate-180" : ""}`} />
            </button>
            ...
            {showGuestPicker && (
              ...
              >
                {[
                  ...
                  <div key={item.key} className="flex items-center justify-between py-3 border-b border-[#222222] last:border-0">
                    <div>
