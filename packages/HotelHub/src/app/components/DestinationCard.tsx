...
export function DestinationCard({ destination, index = 0, size = "md" }: DestinationCardProps) {
  ...
  return (
    ...
    >
      <Link to={`/search?destination=${encodeURIComponent(destination.name)}`}>
        {/* Image */}
        <div className={`relative ${heights[size]} overflow-hidden`}>
          ...
          <div className="absolute bottom-0 left-0 right-0 p-4">
            <div className="flex items-end justify-between">
              <div>
                <h3 className="text-white" style={{ fontSize: "1.2rem", fontWeight: 700, letterSpacing: "-0.01em" }}>
                  {destination.name}
                </h3>
            ...
            <div className="flex items-center gap-1.5 mt-2 pt-2 border-t border-white/10">
              ...
              <span className="text-[#a1a1aa] text-xs">{destination.hotels.toLocaleString()} hotéis disponíveis</span>
            </div>
          </div>
        </div>
      </Link>
